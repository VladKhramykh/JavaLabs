...
{
	CPaintDC dc(this);
	// без удаления невидимых граней
	if (Index == 1) 
		PIR.Draw(dc, PView, WinRect);
	// параметры: к-ты камеры и область в окне windows
	// с удалением невидимых граней
	if (Index == 2)
		PIR.Draw1(dc, PView, WinRect);
	if (Index == 3)
		PIR.ColorDraw(dc, PView, WinRect, RGB(120, 255, 120));
}
...
void CMainWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if ((Index == 1) || (Index == 2) || (Index == 3))
	{
		...
		{
			double d = PView(2) - 3; //PView(2) = 135;
			//if (d >= 0)
			//	PView(2) = d;
			//	//+ 360;
			if (d >= 0)PView(2) = d;
